...
{
    public partial class DieticianSettings : Form
    {
        ...
        private void changePasswordBtn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string commString = "UPDATE DieticianTable SET Password = @password WHERE Username = '" + loginForm.username + "'";
        ...
        {
            if (MessageBox.Show("Are you sure you want to delete your account? Deleting your account is permanent and will remove all content including diet lists, registered dieters and appointments.", "Close Application", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                sqlcon.Open();
                try
                {
                    using (SqlCommand command = new SqlCommand("DELETE FROM DieticianTable WHERE Username = @username", sqlcon))
                    {
                ...
                sqlcon.Close();
                MessageBox.Show("The account has been deleted successfully. Thanks for your contribution. We hope to see you later!", "Goodbye!", MessageBoxButtons.OK);
